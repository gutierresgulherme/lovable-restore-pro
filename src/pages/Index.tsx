...
// Declaração de tipos para o UTMify
declare global {
  interface Window {
    utmify?: {
...
const Index = () => {
  ...
  // Injetar os scripts da UTMify (Pixel + Main Script)
  useEffect(() => {
    console.log("🚀 Iniciando integração UTMify...");
    ...
    // 🔹 Script principal de rastreamento UTMify (captura de UTMs e eventos)
    ...
    console.log("✅ Script principal da UTMify carregado.");

    return () => {
      document.head.removeChild(pixelScript);
      document.body.removeChild(mainScript);
      console.log("🧹 Scripts UTMify removidos.");
    };
  }, []);

  // Evento automático de visualização de produto
  useEffect(() => {
    const timer = setTimeout(() => {
      window.utmify?.track("view_content", {
        product: "Scale Turbo Pro",
        value: 37.90,
      });
      console.log("📊 UTMify: view_content tracked");
    }, 1000);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      window.utmify?.track("add_to_cart", {
        product: "Scale Turbo Pro",
        value: 37.90,
      });
      console.log("📊 UTMify: add_to_cart tracked");

      // Evento de iniciar checkout
      window.utmify?.track("initiate_checkout", {
        product: "Scale Turbo Pro",
        value: 37.90,
      });
      console.log("📊 UTMify: initiate_checkout tracked");
